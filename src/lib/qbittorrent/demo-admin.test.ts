...
import { resetQbittorrentAdminDemo } from "./demo";
import { defaultAdminDemoConfig } from "../demo/admin-catalog";

resetQbittorrentAdminDemo();
...
async function run() {
  ...
  const simpleLogin = await handleQbittorrentCompat(
  ...
  const simpleUi = await handleQbittorrentCompat(
  ...
  assert.ok(simpleCount > 0 && simpleCount < 20, "simple qBittorrent demo stays small");
}

run()
  .then(() => {
    console.log("qbittorrent admin demo tests passed");
  })
  .catch((err) => {
    console.error(err);
    process.exitCode = 1;
